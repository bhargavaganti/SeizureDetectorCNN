...
for i in range(sl_vects.shape[1]):
  ...
  for k in range (0,k):
    print("dist= %u, cat= %u, nid= %u" % (dists[k],cats[k],nids[k]))
...
mat_contents = sio.loadmat('C:\Users\Xinyang\Desktop\sl.mat')
data_arr1 = mat_contents['sl']
data_mtr1 = np.asmatrix(data_arr1)
...
#print("\nLearning three vectors of LENGTH components with identical values")
LENGTH=data_arr1.shape[0];
QUANT = data_arr1.shape[1]
bytearray = c_int * LENGTH
vector=bytearray()
...
mat_contents = sio.loadmat('C:\Users\Xinyang\Desktop\sz.mat')
data_arr2 = mat_contents['sz']
data_mtr2 = np.asmatrix(data_arr2)#

#---------------------------------------------#

LENGTH = data_arr2.shape[0];
QUANT = data_arr2.shape[1]
bytearray = c_int * LENGTH
vector = bytearray()

for j in range(0, QUANT):
  for i in range(0,LENGTH):
    vector[i]=data_arr2[i,j];
    #print vector[i]
...
print("\nDisplaying the neurons' content")
ncount=gvDll.CommittedNeurons();
print ("Committed neurons= " + repr(ncount));
gvDll.Write(1,13,16) #Set network to Save and Restore mode
gvDll.Write(1,12,0) # Resetchain to point at 1st committed neuron
component=bytearray()
for i in range(0,ncount):
...
for j in range(0, QUANT):
  ...
  for i in range (0,k):
    gvDll.Recognize(vector, LENGTH, k, byref(dists), byref(cats), byref(nids));
#  for l in range(k):
#    print("dist= %u, cat= %u, nid= %u" % (dists[l],cats[l],nids[l]))
